...
const VAPID_KEY =
  "BI_RoVYOxXxIG4yh0uhQelkbs8j7iqtRL7CE9MxivlQjdJfa-IoZ7TA8qI2gQNum__DAz1q4fg610qBK8MLd78A"; // 실제 키로 교체
...
export const requestNotificationPermission = async () => {
  ...
  try {
    // 1. Service Worker 등록 (iOS에서는 먼저 등록해야 함)
    ...
    if (permission !== "granted") {
      ...
      if (
        ...
        alert(
          "아이폰에서 알림을 받으려면:\n1. Safari 설정 > 알림 허용\n2. 홈화면에 추가 후 사용\n3. 설정 > Safari > 고급 > 실험적 기능에서 Push API 활성화"
        );
      } else {
    ...
    console.log("✅ 알림 권한 획득:", permission);

    // 3. FCM 토큰 가져오기 (iOS에서는 약간의 지연이 필요할 수 있음)
    ...
    if (currentToken) {
      ...
      if (
        ...
        console.log("📱 iOS 디버깅: Service Worker 상태 확인");
        const swRegistration = await navigator.serviceWorker.getRegistration();
        console.log("📱 Service Worker 등록 상태:", swRegistration);
      }
    ...
    if (
      ...
      console.log("📱 iOS 오류 상세:", error.message);
    }
...
const registerTokenToServer = async (token) => {
  try {
    ...
    console.log(
      "📡 인증 토큰:",
      authToken ? authToken.substring(0, 30) + "..." : "없음"
    );
