...
const FavoriteSection = ({ userId }) => {
  ...
  useEffect(() => {
    const fetchFavorites = async () => {
      ...
      try {
        ...
        // ✅ 1. 즐겨찾기 ID 목록 조회 (authenticatedFetch 사용)
        const idsResponse = await authenticatedFetch(
          `http://localhost:8080/user-service/api/users/${userId}/favorites/ids`
        );

        // ✅ 2. 즐겨찾기 개수 조회 (authenticatedFetch 사용)
        const countResponse = await authenticatedFetch(
          `http://localhost:8080/user-service/api/users/${userId}/favorites/count`
        );
  ...
  const fetchMountainDetails = async (mountainIds) => {
    try {
      const mountainPromises = mountainIds.map(async (mountainId) => {
        try {
          // ✅ authenticatedFetch 사용 (토큰이 필요 없는 공개 API지만 일관성을 위해)
          const response = await authenticatedFetch(
            `http://localhost:8080/mountain-service/search?name=${mountainId}` // 임시: ID 대신 이름으로 검색
          );
          
          if (response.ok) {
            const data = await response.json();
            // searchMountain API는 전체 객체를 반환하므로 mountain 부분만 추출
  ...
  const handleRemoveFavorite = async (mountainId, mountainName) => {
    ...
    try {
      ...
      const response = await authenticatedFetch(
        `http://localhost:8080/user-service/api/users/${userId}/favorites/${mountainId}`,
        { method: 'DELETE' }
      );

      if (response.ok) {
        // UI 업데이트
        setFavoriteMountains(prev => prev.filter(mountain => mountain.id !== mountainId));
        ...
        <div style={statsStyle}>
          ...
          <div style={statItemStyle}>
            ...
            <span style={statValueStyle}>
              {Math.round(favoriteMountains.reduce((sum, m) => sum + m.elevation, 0) / favoriteMountains.length)}m
            </span>
          </div>
        </div>
      )}
