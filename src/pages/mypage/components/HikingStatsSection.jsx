  ...
  useEffect(() => {
    const fetchHikingStats = async () => {
      ...
      try {
        ...
        try {
          const badgeResponse = await axiosInstance.get(
        ...
        try {
          ...
          if (recordsResponse.ok) {
            ...
            // 통계 계산
            const uniqueMountains = new Set(
              recordsData.map((record) => record.mountainName)
            ).size;
            setStats({
        ...
        setError(
          "등산 통계를 불러오는 중 오류가 발생했습니다: " + error.message
        );
      } finally {
        setLoading(false);
      }
    ...
    >
        ...
        >
          <div style={{ textAlign: "center" }}>
            <div style={{ fontSize: "clamp(2rem, 4vw, 3rem)" }}>
              {badgeProfile.stage <= 3
                ? "🥾"
                : badgeProfile.stage <= 6
                ? "🏔️"
                : "👑"}
            </div>
      ...
      <div
        style={{
          display: "grid",
          gridTemplateColumns:
            "repeat(auto-fit, minmax(clamp(8rem, 15vw, 10rem), 1fr))",
      ...
      >
        ...
        >
          ...
          <div
            style={{ fontSize: "clamp(0.8rem, 1.3vw, 0.9rem)", color: "#666" }}
          >
            총 등산 횟수
          </div>
        </div>
          ...
          >
            {hikingRecords.slice(0, 3).map((record) => (
              ...
              >
                {record.photoUrl && (
                  <img
                    src={`${baseUrl}/record-service${record.photoUrl}`}
