...
import { getUserInfo, getToken, isAuthenticated,requireAuth } from 'shared/lib/auth';  // ✅ 추가
...
const MyPage = () => {
 ...
 useEffect(() => {
    ...
    if (!requireAuth("마이페이지를 이용하려면 로그인이 필요합니다. 로그인 페이지로 이동하시겠습니까?")) {
  ...
  useEffect(() => {
    const fetchUserInfo = async () => {
      ...
      try {
        ...
        const response = await fetch(`http://localhost:8080/user-service/api/users/${userId}`, {
          headers: {
            'X-AUTH-TOKEN': token  // ✅ 통일된 헤더 사용
          }
        });
